...
void pcsg_history_free(PCSGHistory* instance) {
    ...
    for(uint16_t i = 0; i < instance->last_index_write; i++) {
        PCSGHistoryItem* item = PCSGHistoryItemArray_get(instance->history->data, i);
...
// UNUSED - This function is defined but never called in current implementation
// The protocol type is stored in the history item but not used anywhere
...
// UNUSED - This function is defined but never called in current implementation
// Protocol name is stored but not used for anything in current code
...
// UNUSED - This function is defined but never called in current implementation
// The raw FlipperFormat data is accessible but not used anywhere in the codebase
...
bool pcsg_history_get_ric(PCSGHistory* instance, uint16_t idx, uint32_t* ric) {
    ...
    // Try to read numeric RIC value first (new format)
    if(flipper_format_read_uint32(item->flipper_string, "RIC", ric, 1)) {
        return true;
    }

    // Fallback: extract from Ric string (old format)
    ...
    // Extract numeric RIC from string like "[P1200]RIC: 0000224 | Num: "
    const char* ric_str = furi_string_get_cstr(temp_ric);
    const char* ric_start = strstr(ric_str, "RIC: ");
    if(ric_start) {
        *ric = strtoul(ric_start + 5, NULL, 10);
        furi_string_free(temp_ric);
        return true;
    }
...
bool pcsg_history_get_message(PCSGHistory* instance, uint16_t idx, FuriString* message) {
    ...
    if(!flipper_format_read_string(item->flipper_string, "Message", message)) {
...
// UNUSED - This function is defined but never called in current implementation
// The menu item text is accessed directly via history indexing instead
