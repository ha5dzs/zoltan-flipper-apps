...
typedef enum {
    ...
    SubmenuIndexPOCSAGPagerSend,
    SubmenuIndexPOCSAGPagerConfig,
    SubmenuIndexPOCSAGPagerAbout,  // SCENE_ACTIVE - About scene is defined and used
} SubmenuIndex;
...
bool pocsag_pager_scene_start_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == SubmenuIndexPOCSAGPagerAbout) {
            ...
            consumed = true;
        } else if(event.event == SubmenuIndexPOCSAGPagerSend) {
            scene_manager_next_scene(app->scene_manager, POCSAGPagerSceneSend);
            consumed = true;
        } else if(event.event == SubmenuIndexPOCSAGPagerConfig) {
