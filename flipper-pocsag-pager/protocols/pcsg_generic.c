...
SubGhzProtocolStatus pcsg_block_generic_serialize(
    ...
    do {
        ...
        // Debug: log Ric before writing
        FURI_LOG_D(TAG, "Writing Ric, size=%zu", instance->result_ric ? furi_string_size(instance->result_ric) : 0);
        if(!flipper_format_write_string(flipper_format, "Ric", instance->result_ric)) {
            FURI_LOG_E(TAG, "Unable to add Ric");
            break;
        }

        // Debug: log Message before writing
        FURI_LOG_D(TAG, "Writing Message, size=%zu", instance->result_msg ? furi_string_size(instance->result_msg) : 0);
        if(!flipper_format_write_string(flipper_format, "Message", instance->result_msg)) {
            FURI_LOG_E(TAG, "Unable to add Message");
            break;
        }

        // Write timestamp (compact datetime format)
        if(!flipper_format_write_uint32(flipper_format, "Timestamp", &instance->timestamp, 1)) {
...
SubGhzProtocolStatus
    ...
    do {
        ...
        // Read timestamp (compact datetime format)
        uint32_t temp_timestamp = 0;
        if(flipper_format_read_uint32(flipper_format, "Timestamp", &temp_timestamp, 1)) {
            instance->timestamp = temp_timestamp;
        } else {
            // Backwards compatibility: if no timestamp, default to 0
            FURI_LOG_W(TAG, "No timestamp found, using default 0");
...
bool subghz_decoder_base_get_ric(SubGhzProtocolDecoderBase* decoder_base, FuriString* output) {
    ...
    do {
        if(subghz_protocol_decoder_base_serialize(decoder_base, ff, NULL) != SubGhzProtocolStatusOk) {
            break;
        }

        flipper_format_rewind(ff);
        if(!flipper_format_read_string(ff, "Ric", temp_data)) {
            break;
        }

        furi_string_set(output, temp_data);
        result = true;
    } while(false);
...
bool subghz_decoder_base_get_message(SubGhzProtocolDecoderBase* decoder_base, FuriString* output) {
    ...
    do {
        ...
        flipper_format_rewind(ff);
        if(!flipper_format_read_string(ff, "Message", temp_data)) {
            break;
        }
